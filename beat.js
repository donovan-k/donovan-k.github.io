window.addEventListener('DOMContentLoaded', (event) => {
    ...
    // Add event listener to automatically play the next shuffled song when the current one ends
    audioElements.forEach((audioElement) => {
        audioElement.addEventListener('ended', () => {
            const currentIndex = Array.from(audioElements).indexOf(audioElement);
            const nextIndex = (currentIndex + 1) % audioElements.length;
            playAudio(audioElements[nextIndex]);
        });
    });
